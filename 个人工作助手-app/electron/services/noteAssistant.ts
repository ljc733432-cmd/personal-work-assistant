...
3. 用 Markdown 无序列表（- 开头）。`,
  ...
  todos: `你是一个待办提取助手。从用户提供的笔记内容中识别可执行的待办事项，并识别哪些是子任务（属于另一个更大任务的细分步骤）。
...
3. **层级判断标准**：当一个待办明确是「为了完成另一个待办而要做的细分步骤」时，作为子任务（缩进 2 空格，紧跟其父任务下方）。判断依据：子任务在逻辑上从属于父任务（如「订餐厅」是「准备约会」的一部分）。
4. **不确定时不嵌套**：如果两个待办只是相关但非从属（如「写周报」和「取快递」），都顶格独立。宁可少嵌套不要错嵌套——用户可在任务页手动调整层级。
5. 子任务必须紧跟其父任务（缩进的那几条属于上方最近的顶格任务）。
6. 如果笔记中没有明确待办，在「## 待办」标题下写「未识别到待办事项」。`,
