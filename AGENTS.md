...
- **进度：v1.20 表格互转已完成**（PRD v1 + v1.2~v1.20 全落地，§15 验收 V-Z1~V-Z9 全绿，**v1 阶段收官**）
- 已完成里程碑：M1 骨架 / M2 对话 / M3 任务 / M4 抽取 / M5 文件+搜索 / M6 跟进 / M12~M17 v1.2~v1.8 工具+UI+日报 / v1.9~v1.20 笔记增强+任务系统+AI产出+图片+截图标注+表格互转。
- **最新打包版**：`release/0.2.5/win-unpacked/个人工作助手.exe`（v1.20）
- ADR 编号到 026（`docs/技术决策记录.md`）
- 详细进度见 `docs/项目总结.md` 第七节。下一步（v2 方向）候选见该节。
